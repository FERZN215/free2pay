...
def offers_kb(posts, n, db, sort="cost"):
    ...
    for i in range(len(posts)):
        ...
        if seller["statistics"]["total"] >0:
            ...
            rat = 0
        cur = InlineKeyboardButton("Продавец: "+seller["local_name"] +"|Класс:"+ ac_t_t(posts[i]["class"]) + "|Уровень: "+str(posts[i]["level"])+"|Цена: "+str(posts[i]["cost"]) + "|Рейтинг:" +str(rat)+"%" , callback_data="acc_offer_id:"+str(posts[i]["_id"]))
        offers_kb.add(cur)
