...
async def message_process_handler(message: types.Message, state: FSMContext, db: Database, dp: Dispatcher, bot: Bot):
    ...
    if message.chat.id == data.get("target"):
        ...
        if await dp.storage.get_state(chat=data.get("source")) != "chat_states:chat_ready" or another_data.get(
                "target") != data.get("target") or another_data.get("source") != data.get("source"):
            return
        else:
            pass
    else:
        another_data = await dp.storage.get_data(chat=data.get("target"))
        if await dp.storage.get_state(chat=data.get("target")) != "chat_states:chat_ready" or another_data.get(
                "target") != data.get("target") or another_data.get("source") != data.get("source"):
            return
        else:
            pass
